...
<Note>
  Every data hook reads `VITE_API_URL` at build time. If unset, it falls back to `http://localhost:3001/api`. This constant is defined at the top of each hook file: `const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001/api'`.
</Note>
...
<Tip>
  Hooks are imported via relative paths to the barrel export. From a page in `src/app/pages/`: `import { useEdificios, useEventos } from '../hooks'`. From a page in `src/app/pages/admin/`: `import { useEdificios } from '../../hooks'`.
</Tip>
